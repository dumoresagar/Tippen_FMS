...
class DistrictListNewSerialzer(serializers.ModelSerializer):
    total_notfound_district_count = serializers.SerializerMethodField('total_notfound_district',read_only=True)
    total_scan_uploaded_count = serializers.SerializerMethodField('total_scan_uploaded',read_only=True)
    scan_uploaded_count = serializers.SerializerMethodField('scan_uploaded',read_only=True)
    rectify_completed_count = serializers.SerializerMethodField('rectify_completed',read_only=True)
    ...
    shape_completed_count = serializers.SerializerMethodField('shape_completed',read_only=True)
    polygon_count = serializers.SerializerMethodField('polygon_count_completed',read_only=True)
    qc_polygon_count = serializers.SerializerMethodField('qc_polygon_count_completed',read_only=True)
    bel_scan_uploaded_count = serializers.SerializerMethodField('bel_scan_uploaded',read_only=True)
    bel_draft_uploaded_count = serializers.SerializerMethodField('bel_draft_uploaded',read_only=True)
    bel_gov_scan_qc_approved_count = serializers.SerializerMethodField('bel_gov_scan_qc_approved',read_only=True)
    bel_gov_draft_qc_approved_count = serializers.SerializerMethodField('bel_gov_draft_qc_approved',read_only=True)
    ...
    def get_agency_id(self):
        return self.context.get('agency_id')
    ...
    def bel_scan_uploaded(self, obj):
        ...
        if agency_id:
            query_set =Document.objects.filter(rectify_agency_id=agency_id,district_code=obj.district_code,bel_scan_uploaded=True).count()
            
        elif User.objects.filter(id=self.context['request'].user.id, user_role__role_name="Super Admin").exists():
            query_set =Document.objects.filter(district_code=obj.district_code,bel_scan_uploaded=True).count()
    ...
    def bel_draft_uploaded(self, obj):
        ...
        if agency_id:
            query_set =Document.objects.filter(qc_agency_id=agency_id,district_code=obj.district_code,bel_draft_uploaded=True).count()
        elif User.objects.filter(id=self.context['request'].user.id, user_role__role_name="Super Admin").exists():
            query_set =Document.objects.filter(district_code=obj.district_code,bel_draft_uploaded=True).count()
    ...
    def bel_gov_scan_qc_approved(self, obj):
        ...
        if agency_id:
            query_set =Document.objects.filter(rectify_agency_id=agency_id,district_code=obj.district_code,bel_gov_scan_qc_approved=True).count()            
        elif User.objects.filter(id=self.context['request'].user.id, user_role__role_name="Super Admin").exists():
            query_set =Document.objects.filter(district_code=obj.district_code,bel_gov_scan_qc_approved=True).count()
    ...
    def bel_gov_draft_qc_approved(self, obj):
        ...
        if agency_id:
            query_set =Document.objects.filter(qc_agency_id=agency_id,district_code=obj.district_code,bel_gov_draft_qc_approved=True).count()         
        elif User.objects.filter(id=self.context['request'].user.id, user_role__role_name="Super Admin").exists():
            query_set =Document.objects.filter(district_code=obj.district_code,bel_gov_draft_qc_approved=True).count()
    ...
    def polygon_count_completed(self, obj):
        ...
        if agency_id:
            query_set =Document.objects.filter(digitize_agency_id=agency_id,district_code=obj.district_code).aggregate(total_amount=Sum('polygon_count'))['total_amount']
        elif User.objects.filter(id=self.context['request'].user.id, user_role__role_name="Super Admin").exists():
            query_set =Document.objects.filter(district_code=obj.district_code).aggregate(total_amount=Sum('polygon_count'))['total_amount']
    ...
    def qc_polygon_count_completed(self, obj):
        agency = User.objects.filter(id=self.context['request'].user.id).values_list('agency',flat=True)        
        agency_id = self.get_agency_id()

        if agency_id:
            query_set =Document.objects.filter(qc_agency_id=agency_id,district_code=obj.district_code).aggregate(total_amount=Sum('qc_polygon_count'))['total_amount']

        elif User.objects.filter(id=self.context['request'].user.id, user_role__role_name="Super Admin").exists():
            query_set =Document.objects.filter(district_code=obj.district_code).aggregate(total_amount=Sum('qc_polygon_count'))['total_amount']
    ...
    def rectify_completed(self, obj):
        ...
        if agency_id:
            query_set = Document.objects.filter(rectify_agency_id=agency_id, district_code=obj.district_code, rectify_completed_date__isnull=False).count()
    ...
    def digitize_completed(self, obj):
        ...
        if agency_id:
            query_set = Document.objects.filter(digitize_agency_id=agency_id,district_code=obj.district_code,digitize_completed_date__isnull=False).count()
        elif User.objects.filter(id=self.context['request'].user.id, user_role__role_name="Super Admin").exists():
            query_set = Document.objects.filter(district_code=obj.district_code,digitize_completed_date__isnull=False).count()
    ...
    def qc_completed(self, obj):
        ...
        if agency_id:
            query_set = Document.objects.filter(qc_agency_id=agency_id,district_code=obj.district_code,qc_completed_date__isnull=False).count()
        elif User.objects.filter(id=self.context['request'].user.id, user_role__role_name="Super Admin").exists():
            query_set = Document.objects.filter(district_code=obj.district_code,qc_completed_date__isnull=False).count()
    ...
    def pdf_completed(self, obj):
        ...
        if agency_id:
            query_set = Document.objects.filter(qc_agency_id=agency_id,district_code=obj.district_code,pdf_completed_date__isnull=False).count()
        elif User.objects.filter(id=self.context['request'].user.id, user_role__role_name="Super Admin").exists():
            query_set = Document.objects.filter(district_code=obj.district_code,pdf_completed_date__isnull=False).count()
    ...
    def shape_completed(self, obj):
        ...
        if agency_id:
            query_set = Document.objects.filter(shape_agency_id=agency_id,district_code=obj.district_code,shape_completed_date__isnull=False).count()
        elif User.objects.filter(id=self.context['request'].user.id, user_role__role_name="Super Admin").exists():
            query_set = Document.objects.filter(district_code=obj.district_code,shape_completed_date__isnull=False).count()
...
class BelDocumentListSerializer(serializers.ModelSerializer):
    current_status = serializers.SerializerMethodField(read_only=True)

    class Meta:
        model = Document
        fields = ["id","barcode_number","file_name","district_code","village_code","taluka_code","map_code","polygon_count","qc_polygon_count","current_status",
                  "bel_scan_uploaded","bel_draft_uploaded","bel_gov_scan_qc_approved","bel_gov_draft_qc_approved"]
