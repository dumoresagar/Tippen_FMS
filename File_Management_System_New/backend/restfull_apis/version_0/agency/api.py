    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if User.objects.filter(id=self.request.user.id, user_role__role_name="Super Admin"):
                user_obj = User.objects.filter(department=kwargs.get('pk'),active_status="Active")
            elif User.objects.filter(id=self.request.user.id, user_role__role_name="Team Admin"):
                user_obj = User.objects.filter(agency__team__in=team_ids,department=kwargs.get('pk'),active_status="Active")
            elif User.objects.filter(id=self.request.user.id, user_role__role_name="Agency Admin"):
                user_obj = User.objects.filter(agency__in=agency_ids,department=kwargs.get('pk'),active_status="Active")
