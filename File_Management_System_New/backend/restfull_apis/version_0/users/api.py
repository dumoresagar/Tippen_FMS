...
class AdminLoginAPI(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Logout previous session if exists (except for SuperAdmin)
        if user.user_role and user.user_role.role_name != "Super Admin":
            if user.session_key:
                try:
        ...
        # Save the new session key (except for SuperAdmin, who may already have a session)
...
class LogoutAllUsersExceptSuperAdminAPI(APIView):
    permission_classes = [IsAdminUser]  # Only Admins can perform this action
    authentication_classes = [TokenAuthentication]

    def post(self, request):
        # Get all Super Admin users
        super_admins = User.objects.filter(is_superuser=True).values_list("id", flat=True)

        # Remove session keys and logout all users except Super Admins
        users_to_logout = User.objects.exclude(id__in=super_admins)
        users_to_logout.update(session_key=None)  # Remove stored session keys

        # Delete all session records except for Super Admins
        Session.objects.exclude(session_key__in=User.objects.filter(id__in=super_admins).values_list('session_key', flat=True)).delete()

        # Delete authentication tokens except for Super Admins
        Token.objects.exclude(user_id__in=super_admins).delete()

        return Response({"message": "All users (except Super Admins) have been logged out successfully."}, status=status.HTTP_200_OK)