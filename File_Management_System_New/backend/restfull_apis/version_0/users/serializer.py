...
class AdminLoginSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not user:
            ...
            raise serializers.ValidationError({"message": "The user account has been deactivated. Please contact support Admin"})

           
        
        # Force logout before proceeding
        if user.session_key:
            Session.objects.filter(session_key=user.session_key).delete()
            user.session_key = None
            user.save()

        # Delete user's authentication token (if using token authentication)
        # Token.objects.filter(user=user).delete()
        if not (user.user_role and user.user_role.role_name == "Super Admin"):
            Token.objects.filter(user=user).delete()

    
        # Check if the user is a SuperAdmin
        if user.user_role and user.user_role.role_name == "Super Admin":
            # Bypass session check for SuperAdmin
            attrs['user'] = user
            return attrs

        # Check if the user has an active session
        if user.session_key:
            session_exists = Session.objects.filter(session_key=user.session_key).exists()
            if session_exists:
                raise serializers.ValidationError(
